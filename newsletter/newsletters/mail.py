...
def send_email(from_email="info@eawaters.com",to_emails=[],subject="",content="",ccs=[],bccs=[],attachments=[]):
    ...
    tos=[email for email in to_emails]
    bccs=[email for email in bccs]
    ccs=[email for email in ccs]
    message = EmailMessage(subject,content,from_email,tos,bccs,reply_to=['no-reply@kipya-africa.com'],headers={'Message-ID': str(uuid.uuid4()) })
